...
(function () {
  ...
  const sessionId = globalConfig.SESSION_ID;
  if (!sessionId) {
    log('Не задан session_id в CONFIG');
    return;
  }
  ...
  class ChunkedRecorder {
    constructor(options = {}) {
      ...
      this.onLimitReached = options.onLimitReached || defaultNotifyLimit;
    ...
    async start({ stream } = {}) {
      ...
      try {
        if (stream) {
          this._stream = stream;
        } else if (typeof this.streamProvider === 'function') {
          this._stream = await this.streamProvider();
        } else {
    ...
    async _handleChunk(event) {
      ...
      if (this._chunkIndex >= this.maxChunks) {
        if (!this._limitReached) {
          ...
          try {
            this.onLimitReached(this.maxChunks);
          } catch (notifyError) {
  ...
  async function withRetry(task, options = {}) {
    const cfg = { retries: 4, minTimeout: 500, factor: 2, maxTimeout: 5000, ...options };
    let attempt = 0;
  ...
  function ensureStreamAvailable() {
    const stream = window.__mediaStream;
    if (!stream) {
      throw new Error('Медиа-поток не инициализирован: ожидается window.__mediaStream');
    }
    return stream;
  }
  ...
  if (buttons.start) {
    buttons.start.onclick = async () => {
      try {
        const stream = ensureStreamAvailable();
        recorder = new ChunkedRecorder({
          sessionId,
          kind: 'video',
          timeslice: globalConfig.MEDIA_TIMESLICE_MS,
          maxChunks: globalConfig.MEDIA_MAX_CHUNKS,
          mimeType: 'video/webm;codecs=vp9,opus',
          streamProvider: () => stream,
          onChunkUploaded: ({ index }) => log(`Загружен фрагмент ${index + 1}`),
          onLimitReached: (max) => log(`Достигнут лимит из ${max} фрагментов`),
        });
  ...
  if (buttons.stop) {
    buttons.stop.onclick = async () => {
      ...
      try {
        const result = await recorder.stop({ finalize: true });
        log(`Финализация завершена: ${JSON.stringify(result)}`);
      } catch (error) {
        defaultHandleError(error);
      }
    };
  ...
  if (buttons.next) {
    buttons.next.onclick = () => log('NEXT QUESTION (placeholder)');
  }
})();
